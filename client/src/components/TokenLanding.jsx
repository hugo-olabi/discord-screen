import { createSignal, Show } from 'solid-js';
import { A } from '@solidjs/router';

export default function TokenLanding(props) {
  const [tokenInput, setTokenInput] = createSignal('');

  const parseToken = (input) => {
    if (!input) return '';
    const trimmed = input.trim();
    if (trimmed.includes('token=')) {
      try {
        const urlParams = new URLSearchParams(trimmed.split('?')[1]);
        return urlParams.get('token') || trimmed;
      } catch {
        return trimmed;
      }
    }
    return trimmed;
  };

  const handleJoin = (e) => {
    e.preventDefault();
    const cleanToken = parseToken(tokenInput());
    if (cleanToken) {
      props.onJoinToken?.(cleanToken);
    }
  };

  return (
    <div class="token-landing">
      <div class="token-card">
        <header class="token-card-header">
          <div class="brand-badge-lg">
            <span class="brand-dot-lg"></span>
            <h1 class="brand-title">StreamRoom</h1>
          </div>
          <p class="token-card-sub">
            High-performance zero-latency screen and audio sharing. Enter a short stream token to join.
          </p>

          <div class="auth-bar">
            <Show
              when={props.user()?.isDiscord}
              fallback={
                <div class="auth-box-guest">
                  <div class="guest-info">
                    <span class="muted">Signed in as:</span>
                    <strong>{props.user()?.name || 'Guest'}</strong>
                  </div>
                  <button class="btn discord-login-btn" onClick={() => props.onDiscordLogin?.()}>
                    <svg viewBox="0 0 24 24" class="icon-sm" fill="currentColor">
                      <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028c.462-.63.874-1.295 1.226-1.994.021-.041.001-.09-.041-.106a13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.061 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.893.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.028z" />
                    </svg>
                    Sign in with Discord
                  </button>
                </div>
              }
            >
              <div class="auth-box-discord">
                <div class="discord-user-badge">
                  <Show when={props.user()?.avatar} fallback={<div class="avatar-fallback">{props.user()?.name?.[0]}</div>}>
                    <img src={props.user()?.avatar} alt="Discord Avatar" class="avatar-discord" />
                  </Show>
                  <div class="discord-user-meta">
                    <span class="discord-tag">Discord Verified</span>
                    <strong>{props.user()?.name}</strong>
                  </div>
                </div>
                <button class="btn sm" onClick={() => props.onDiscordLogout?.()}>
                  Sign out
                </button>
              </div>
            </Show>
          </div>
        </header>

        <form onSubmit={handleJoin} class="token-form">
          <label class="field-col">
            <span>Enter Stream Token or URL</span>
            <div class="token-input-row">
              <input
                type="text"
                value={tokenInput()}
                onInput={(e) => setTokenInput(e.target.value)}
                placeholder="e.g. x9K2p7 or https://.../?token=x9K2p7"
                class="token-input"
                autocomplete="off"
                required
              />
              <button type="submit" class="btn go lg-btn">
                Join Stream
              </button>
            </div>
          </label>
        </form>

        <div class="token-card-divider">
          <span>OR</span>
        </div>

        <div class="token-create-sec">
          <button class="btn outline-btn full-btn" onClick={() => props.onOpenCreate?.()}>
            <svg viewBox="0 0 24 24" class="icon-sm" fill="currentColor">
              <path d="M19 13h-6v6h-2v-6H5v-2h6V5h2v6h6v2z" />
            </svg>
            Create New Stream (Generates Token)
          </button>
        </div>

        <footer class="token-card-footer">
          <A href="/privacy-policy" class="footer-link">Privacy Policy</A>
          <span class="dot-sep">•</span>
          <A href="/terms-of-service" class="footer-link">Terms of Service</A>
        </footer>
      </div>
    </div>
  );
}
